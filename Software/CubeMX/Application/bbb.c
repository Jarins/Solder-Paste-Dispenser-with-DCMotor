...
uint8_t bbb_Update(void){
	...
	for(uint8_t i=0; i< UT_SIZEOFARRAY(BtnList); i++){											//Update status of all the buttons in the list
		...
		//if( LL_GPIO_IsInputPinSet(BtnList[i]->Port, BtnList[i]->Pin) ){
		//if(GPIOC -> IDR & GPIO_PIN_13) //Input with pullup, switch to gnd (0 when pressed, 1 when released). !!!BUT!!! below in the code 0 means not pressed, 1 means pressed - called "normalized logic"
